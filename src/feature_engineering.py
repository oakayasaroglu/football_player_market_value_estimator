...
class FeatureEngineer:
    ...
    def engineer_categorical_features(self, df):
        print("Engineering categorical features (encoding and dropping)...")
        
        # 1. One-Hot Encoding for low cardinality features
        low_card_cols = ['foot', 'position', 'main_position']
        existing_low_card = [col for col in low_card_cols if col in df.columns]
        if existing_low_card:
            df = pd.get_dummies(df, columns=existing_low_card, dummy_na=False, dtype=int)
            
        # 2. Frequency Encoding for high cardinality features
        high_card_cols = ['current_club_name', 'citizenship', 'country_of_birth']
        for col in high_card_cols:
            if col in df.columns:
                freq = df[col].value_counts(normalize=True)
                df[f'{col}_freq'] = df[col].map(freq).fillna(0)
                
        # 3. Drop original text and date columns that are no longer needed
        cols_to_drop = [
            'player_name', 'date_of_birth', 'contract_expires', 'joined', 
            'last_injury_date', 'last_transfer_date', 'date', 'date_unix',
            'current_club_name', 'citizenship', 'country_of_birth'
        ]
...
if __name__ == "__main__":
    ...
    cols_to_show = ['player_id', 'age', 'career_goals_per_game', 'recent_goals_per_game', 'avg_injury_days']
